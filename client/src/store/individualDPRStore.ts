...
export interface IndividualDPRData {
  ...
  dprId?: string;
  projectId?: string;
  matchedSchemeCode?: string | null;
  ventureMatchAnswers?: any;
  schemeExtras?: Record<string, any>;
}

interface IndividualDPRState {
  data: IndividualDPRData;
  ...
  setMatchedSchemeCode: (code: string | null) => void;
  setVentureMatchAnswers: (answers: any) => void;
  setSchemeExtras: (extras: Record<string, any>) => void;
  applyPrefill: (partial: Record<string, any>) => void;
  resetData: () => void;
  loadDataFromProject: (projectData: any, dprData?: any) => void;
}

const defaultData: IndividualDPRData = {
  currentStep: 1,
  isDraft: true,
  lastSaved: new Date(),
...
export const useIndividualDPRStore = create<IndividualDPRState>()(
  (set, get) => ({
    data: defaultData,
    ...
    setStepData: (step, stepData) => {
      set((state) => ({
        data: {
          ...state.data,
          [`step${step}`]: stepData,
          // lastSaved: new Date(), // Removed auto-save timestamp
        },
      }));
    },
    ...
    getStepData: (step) => {
      const state = get();
      return state.data[`step${step}` as keyof IndividualDPRData] || {};
    },
    ...
    setMatchedSchemeCode: (code) => {
      set((state) => ({
        data: { ...state.data, matchedSchemeCode: code },
      }));
    },

    setVentureMatchAnswers: (answers) => {
      set((state) => ({
        data: { ...state.data, ventureMatchAnswers: answers },
      }));
    },

    setSchemeExtras: (extras) => {
      set((state) => ({
        data: { ...state.data, schemeExtras: extras },
      }));
    },

    applyPrefill: (partial) => {
      set((state) => {
        const next = { ...state.data };
        Object.entries(partial || {}).forEach(([key, value]) => {
          if (key.startsWith('step') && value && typeof value === 'object') {
    ...
    resetData: () => {
      set({ data: { ...defaultData, lastSaved: new Date() } });
    },
    ...
    loadDataFromProject: (projectData: any, dprData?: any) => {
      ...
      const loadedData: IndividualDPRData = {
